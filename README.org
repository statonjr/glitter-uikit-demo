...
{:paths ["src"]
 ...
 :deps {io.github.burinc/glitter
        ...
        {:git/url "https://github.com/jolt-lang/http-client"
         :git/sha "04825632ed96a77a1c6ba1921c0a31280a3daade"}
...
(s/def ::on-open fn?)   ; optional: runs on every open/reopen of the window
    ...
    (when ex
      (if-let [win (@example-windows id)]
        ...
        (let [win (u/window-new title width height)]
          ...
          (place-near-pointer! win)
          (u/window-show! win)))
      ;; :on-open runs on every open AND reopen — Live Currency uses it to
      ;; fetch fresh rates each time its window comes forward.
...
- [[https://github.com/burinc/glitter][glitter]] — the Replicant-style reconciler; owns =glitter.core= and the
  IRender/IMemory renderer seam.
- [[https://github.com/burinc/glitter-uikit][glitter-uikit]] — the AppKit renderer this demo drives. Its
  [[https://github.com/burinc/glitter-uikit/blob/main/src/glitter_uikit/ffi.clj][ffi.clj]] docstring is the best short course on calling =objc_msgSend= from
