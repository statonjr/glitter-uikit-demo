...
(defonce state (atom {:count 0 :bg nil}))
#+end_src
    ...
    (swap! w/actions assoc-in [panel :click]
           (fn [sender]
             (when-let [rgba (nscolor->rgba (u/objc-msg-send-0 sender (u/sel "color")))]
               (execute-actions nil [[:action/set-bg rgba]]))))
...
self-releasing on close and our cached pointer would dangle on reopen; and the
prefs view is ordinary hiccup on the SAME state atom, so the swatch label
re-renders live as the panel drags.
...
digraph dataflow {
  ...
  main  -> dispatch [label="[[:action/inc]] etc."];
